...
CREATE TABLE bootcamp (
    id_bootcamp  VARCHAR(2) NOT NULL,
    nombre       VARCHAR(64) NOT NULL,
    duración     VARCHAR(64) NOT NULL,
...
CREATE TABLE bootcamp_campus (
    bootcamp_id_bootcamp VARCHAR(2) NOT NULL,
    campus_id_campus     VARCHAR(2) NOT NULL
);

CREATE TABLE bootcamp_estudiante (
    bootcamp_id_bootcamp     VARCHAR(2) NOT NULL,
    estudiante_id_estudiante VARCHAR(16) NOT NULL
);

CREATE TABLE campus (
    id_campus VARCHAR(2) NOT NULL,
    nombre    VARCHAR(64) NOT NULL,
...
CREATE TABLE evaluable (
    ...
    modulo_id_módulo VARCHAR(4) NOT NULL
);
...
CREATE TABLE matriculado (
    id                       VARCHAR(2),
    numeroplaza              VARCHAR(3),
    añomatricula             VARCHAR(4),
    bootcamp_id_bootcamp     VARCHAR(2) NOT NULL,
    estudiante_id_estudiante VARCHAR(16) NOT NULL,
    pago_id_pago             VARCHAR(256) NOT NULL
);

CREATE TABLE metodo_pago (
    id_metodopago            VARCHAR(256) NOT NULL,
    estudiante_id_estudiante VARCHAR(16) NOT NULL,
    tarjeta_id_tarjeta       VARCHAR(256) NOT NULL,
    paypal_login_paypal      VARCHAR(256) NOT NULL
);

ALTER TABLE metodo_pago ADD CONSTRAINT metodo_pago_pk PRIMARY KEY ( id_metodopago );

CREATE TABLE modulo (
    id_módulo            VARCHAR(4) NOT NULL,
    nombre               VARCHAR(32) NOT NULL,
    descripcion          VARCHAR(300) NOT NULL,
    estado               VARCHAR(32) NOT NULL,
    bootcamp_id_bootcamp VARCHAR(2) NOT NULL
...
CREATE TABLE pago (
    id_pago                  VARCHAR(256) NOT NULL,
    estado                   VARCHAR(10) NOT NULL,
    fecha__pago              DATE NOT NULL,
    precio_bootcamp          VARCHAR(6) NOT NULL,
    otros_detalles           VARCHAR(300) NOT NULL,
...
CREATE TABLE tarjeta (
    id_tarjeta   VARCHAR(256) NOT NULL,
